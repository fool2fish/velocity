...
function renderVarRecursive(data, conf, fullPath /*private params*/, indent, upFullPath) {
  ...
  for (downFullPath in morePaths) {
    renderVarRecursive(data, conf, downFullPath, indent + 2, fullPath);
  }
}
...
function renderDepRecursive(data, conf, fullPath /*private params*/, indent, upFullPath) {
  ...
  Object.keys(morePaths).forEach(function(downFullPath) {
    renderDepRecursive(data, conf, downFullPath , indent + 2, fullPath);
  });
}
...
function renderDepPlain(data, conf) {
  ...
  Object.keys(data).forEach(function(fullPath) {
    ...
    if (amount) {
      if (verbose || !reverse) console.log(item.relPath + occur(keys));
    } else {
      if (verbose || reverse) console.log(colorful.gray(item.relPath));
    }
  });
  console.log();
}

...
Engine.prototype.render = function(context, template) {
  ...
  if (fullPath) {
    if (fullPath in this.ast) {
      ast = this.ast[fullPath]
    } else {
      ast = parser.parse(fs.readFileSync(fullPath, {encoding: this.cfg.encoding}))
      this.ast[fullPath] = ast
    }
  } else {
    ast = parser.parse(template)
  }

  var rt = this[ast.type](context || {}, ast)
  if (arguments.length === 1) this.symbol = {reference:{}, macro:{}}
  return rt
}

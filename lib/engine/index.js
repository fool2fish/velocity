...
Engine.prototype.pushContext = function(context) {
  if (!this.context) this.topContext = context
...
Engine.prototype.Foreach = function(node) {
  ...
  if (leftObj.type !== 'Identifier') {
    logger.error('Left operand of #foreach must be an id <%s>.', key)
  }
  ...
  args.forEach(function(arg) {
    ...
    if (isNotEmptyStr(relPath)) {
      ...
      if (fullPath) {
        ...
        logger.error('Invalid include file path <%s> .', relPath)
      }
    } else {
      logger.error('Include file path is required.')
    }
  })
  return rt
}
...
Engine.prototype.Macro = function(node) {

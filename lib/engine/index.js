...
Engine.prototype.render = function(context) {
  ...
  } else if (result.stats === STATS.BREAK) {
    result.stats = STATS.FAIL
    result.message = 'Illegal #break.'
  }
  return result
}
...
// print fail info in terminal
Engine.printFailInfo = function(data) {
  var templ = data.template
  console.log()
  ...
  if (templ.isFile) {
    ...
    var len = templ.raw.length
    f = templ.raw.substr(0, 40).replace(/\n/g, '\\n')
    if (len > 40) f += '...'
  }
  console.log('File:', f, '(line:', data.pos.first_line, ')')
  console.log()
  console.log('  ' + templ.lines[data.pos.first_line - 1])
  console.log('  ' + new Array(data.pos.first_column + 1).join('-') + '^')
  console.log()
}

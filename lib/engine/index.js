...
Engine.prototype.getMacro = function(obj) {
  var that = this
  var content = obj.isFile ? util.readFile(obj.fullPath, this.cfg.encoding) : obj.raw
...
Engine.prototype.render = function(context) {
  ...
  if (result.stats === STATS.SUCCESS || result.stats === STATS.STOP) {
    ...
    return {
      ...
      file: result.template.fullPath || trim(result.template.raw),
      lines: result.template.lines.filter(function(line, idx) {
        return idx + 1 >= pos.first_line && idx + 1 <= pos.last_line
      }),
      pos: pos
    }
  }
...
Engine.prototype.pushContext = function(context) {
  if (!this.context) {
    context.__macro = common.mix(null, this.macro)

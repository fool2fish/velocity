...
module.exports = {
  If: function(node) {
    ...
    if (testr.stats === STATS.BREAK || testr.stats === STATS.STOP) {
      return this.initFailInfo(this.BREAK_STOP_ERR, test.pos)
    }
  ...
  Foreach: function(node) {
    ...
    if (rightr.stats === STATS.BREAK || rightr.stats === STATS.STOP) {
      return this.initFailInfo(this.BREAK_STOP_ERR, right.pos)
    }
    ...
    for (var i = 0; i < list.length; i++) {
      ...
      if (cr.stats === STATS.SUCCESS) {
        ...
        result.value += cr.value
        break
      } else if (cr.stats === STATS.STOP) {
        result.stats = cr.stats
        result.value += cr.value
        break
      } else {
        this.mergeFailInfo(result, cr)
        break
      }
    }

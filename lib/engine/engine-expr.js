...
module.exports = {
  Range: function(node) {
    ...
    if (!util.isInteger(startr.value))
      return this.initFailInfo('Start of range is not an integer.', start.body)
    ...
    if (!util.isInteger(endr.value))
      return this.initFailInfo('End of range is not an integer.', end.body)
  ...
  DString: function(node) {
    var result = this.render(null, node.value)

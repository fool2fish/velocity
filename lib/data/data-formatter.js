...
function dump2str(data){
    return data.replace(/\'\s*#foreach/g, '#foreach')
            .replace(/#end\s*\'/g, '#end')
}

function dumptraversal(data,key,prefix){
    ...
    if (utilx.isArray(data)){
        ...
        data.forEach(function(i,k){
            ...
            goon ?(function(){
                var inner = '{'
                var d = dumptraversal(i,'value','')
                for(j in d){
                    inner += (inner=='{'?'':',') +j+':'
                    if(utilx.isArray(d[j])){
                        inner += '['+d[j]+']'
                    }else{
                        inner += '"$'+d[j]+'"'
                    }
                ...
                inner +='}'
                rt.push(head + count+inner+end)
            })() : rt.push(head + count + '"$value"' + end)
        })
        return rt
    }
}
